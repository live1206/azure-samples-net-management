...
app.MapMethods("/{**resourcePath}", new[] { "GET" }, (string resourcePath) =>
{
    ...
    if (id.EndsWith("/providers/Microsoft.Network/virtualNetworks", StringComparison.OrdinalIgnoreCase))
    {
        string prefix = id + "/";
        var values = new JsonArray(resources
            .Where(pair => pair.Key.StartsWith(prefix, StringComparison.OrdinalIgnoreCase)
                && !pair.Key.Substring(prefix.Length).Contains('/'))
            .Select(pair => pair.Value.DeepClone())
            .ToArray());
        return ArmJson(new JsonObject { ["value"] = values });
    }
